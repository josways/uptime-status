...
function UptimeRobot({ apikey }) {
  ...
  useEffect(() => {
    if (!config) return;
    GetMonitors(apikey, config.CountDays).then(setMonitors);
  }, [apikey, config]);
...
{site.daily.map((data, index) => {
          ...
          <div className='summary'>
            ...
            <span>
              {site.total.times
                ? `最近 ${config.CountDays} 天故障 ${site.total.times} 次，累计 ${formatDuration(site.total.duration)}，平均可用率 ${site.average}%`
                : `最近 ${config.CountDays} 天可用率 ${site.average}%`}
            </span>
